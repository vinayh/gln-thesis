...
class GatedPlotter:
    ...
    def gen_xy_grid(self):
        ...
        with torch.no_grad():
            ...
            xy = torch.tensor(
                np.stack([XX.ravel(), YY.ravel(), np.ones(XX.size)]).T,
                dtype=torch.float,
            )
            return xy, XX, YY
    ...
    def save_animation(self, filename):
        anim = FuncAnimation(
            self.plot, self.plot_step, frames=enumerate(self.Z_out_all)
        )
