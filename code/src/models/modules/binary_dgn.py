...
class BinaryDGN(LightningModule):
    ...
    def gated_layer(self, h, s, y, l_idx, is_train):
        ...
        weights = torch.bmm(c.float().permute(2, 0, 1), self.W[l_idx]).permute(
            1, 0, 2)  # [batch_size, layer_dim, input_dim]
        h_out = torch.bmm(weights, h.unsqueeze(2)).squeeze(
            2)  # [batch_size, layer_dim]
        if is_train:
            r_out = torch.sigmoid(h_out)
            r_out_clipped = torch.clamp(
                r_out, self.pred_clip, 1-self.pred_clip)
            # [batch_size, layer_dim]
            ...
            w_delta = torch.bmm(c.float().permute(
                2, 1, 0), w_grad2.permute(1, 0, 2))
    ...
    def forward(self, s, y, is_train: bool):
        ...
        s = torch.cat(
            [s, torch.ones(batch_size, 1, device=self.device)], dim=1)
