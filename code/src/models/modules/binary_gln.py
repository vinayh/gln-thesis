...
class HalfSpace(LightningModule):
    def __init__(self, s_dim, layer_size, num_subcontexts, ctx_bias=True):
        ...
        if self.ctx_bias:
            ...
            input_dim = s_dim
        self.ctx_weights = torch.empty(num_subcontexts, input_dim, layer_size, device=self.device).normal_(mean=0.5, std=1.0)
...
class BinaryGLN(LightningModule):
    def __init__(self, hparams: dict):
        ...
        for i in range(len(self.l_sizes)-1):  # Add ctx and w for each layer until the single output neuron layer
            ...
            self.W.append(torch.full((curr_layer_dim,
                                      ...
                                      next_layer_dim),
                                     1.0/curr_layer_dim, device=self.device))
    ...
    def forward(self, s, y):
        ...
        """
        batch_size = s.shape[0]
        s = s.flatten(start_dim=2).squeeze(dim=1)
        if self.ctx_bias:
            s = torch.cat([s, torch.ones(batch_size, 1)], dim=1)
        self.t += batch_size
        self.lr = min(0.6, 5000/self.t)
        with torch.no_grad():
